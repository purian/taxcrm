...
class FetchSalesDataService
  ...
  def prepare_sale_record(record)
    {
      Source: decode_html_entities(record.dig('Source', 'objectId')),
      OtherSource: decode_html_entities(record['OtherSource']),
      LeadOwnerId_username: decode_html_entities(record.dig('LeadOwnerId', 'username')),
      LeadOwnerId_name: decode_html_entities(record.dig('LeadOwnerId', 'name')),
      LeadOwnerId_active: record.dig('LeadOwnerId', 'active'),
      LeadOwnerId_job: decode_html_entities(record.dig('LeadOwnerId', 'job')),
      LeadOwnerId_phone: decode_html_entities(record.dig('LeadOwnerId', 'phone')),
      LeadOwnerId_extension: decode_html_entities(record.dig('LeadOwnerId', 'extension')),
      LeadOwnerId_last_success_login: parse_date(record.dig('LeadOwnerId', 'last_success_login')),
      ...
      WorkStatus_Name: decode_html_entities(record.dig('WorkStatus', 'Name')),
      FamilyStatus_Name: decode_html_entities(record.dig('FamilyStatus', 'Name')),
      SoldProperty6Years: decode_html_entities(record.dig('SoldProperty6Years', 'objectId')),
      PropertyTypeSold_Name: decode_html_entities(record.dig('PropertyTypeSold', 'Name')),
      AdditionalSellers_Name: decode_html_entities(record.dig('AdditionalSellers', 'Name')),
      NumberOfHeirs: record['NumberOfHeirs'],
      AdditionalAsset: decode_html_entities(record.dig('AdditionalAsset', 'objectId')),
      ...
      updatedBy: decode_html_entities(record.dig('updatedBy', 'objectId')),
      createdBy: decode_html_entities(record.dig('createdBy', 'objectId')),
      Number: record['Number'],
      StatusId: decode_html_entities(record.dig('StatusId', 'objectId')),
      DocStatus: decode_html_entities(record.dig('DocStatus', 'objectId')),
      Number2: record['Number2'],
      Lawyers_Name: decode_html_entities(record.dig('Lawyers', 'Name')),
      Lawyers_Address: decode_html_entities(record.dig('Lawyers', 'Address')),
      Lawyers_OwnerId: decode_html_entities(record.dig('Lawyers', 'OwnerId', 'objectId')),
      Lawyers_PhoneNumber: decode_html_entities(record.dig('Lawyers', 'PhoneNumber')),
      Lawyers_Description: decode_html_entities(record.dig('Lawyers', 'Description')),
      Lawyers_OfficePhone: decode_html_entities(record.dig('Lawyers', 'OfficePhone')),
      Lawyers_Email: decode_html_entities(record.dig('Lawyers', 'Email')),
      Lawyers_LinkingFactor: decode_html_entities(record.dig('Lawyers', 'LinkingFactor')),
      Lawyers_Comment: decode_html_entities(record.dig('Lawyers', 'Comment')),
      Lawyers_StatusLaw: decode_html_entities(record.dig('Lawyers', 'StatusLaw', 'objectId')),
      LeadStatusId_Name: decode_html_entities(record.dig('LeadStatusId', 'Name')),
      LeadStatusId_StateId: decode_html_entities(record.dig('LeadStatusId', 'StateId', 'objectId')),
      DateBecomeCustomer: parse_date(record['DateBecomeCustomer']),
      ChangedLeadToClient_username: decode_html_entities(record.dig('ChangedLeadToClient', 'username')),
      ChangedLeadToClient_email: decode_html_entities(record.dig('ChangedLeadToClient', 'email')),
      ChangedLeadToClient_name: decode_html_entities(record.dig('ChangedLeadToClient', 'name')),
      ChangedLeadToClient_active: record.dig('ChangedLeadToClient', 'active'),
      ChangedLeadToClient_extension: decode_html_entities(record.dig('ChangedLeadToClient', 'extension')),
      ChangedLeadToClient_job: decode_html_entities(record.dig('ChangedLeadToClient', 'job')),
      ChangedLeadToClient_phone: decode_html_entities(record.dig('ChangedLeadToClient', 'phone')),
      ChangedLeadToClient_last_success_login: parse_date(record.dig('ChangedLeadToClient', 'last_success_login')),
  ...
  def safe_dig(hash, *keys)
    keys.reduce(hash) { |acc, key| acc[key] }
  end
end